# to fix
# dns foawarders dont work
#  asked for username when that part comes up
        # VERBOSE: PowerShell Direct responding on VM [meraki-dc1]. Setting DNS Fowarders
        # Windows PowerShell credential request
        # Enter your credentials.
        # Password for user meraki.airideas.net\administrator: ********
        # VERBOSE: DC Provisioning Complete!!!!

# modifed 8.23.2018
# added vlans to the vm creation
# changed nexthop on vms to hp 2910 vlan core switch 
...
# set firewall rule on rdp
# Get-NetFirewallRule -DisplayName "Remote Desktop*" | Set-NetFirewallRule -enabled true
# add dhcp  https://www.faqforge.com/windows/configure-dhcp-powershell/
    # Install-WindowsFeature -Name 'DHCP' –IncludeManagementTools
    # Add-DhcpServerV4Scope -Name "DHCP Scope" -StartRange 192.168.128.125 -EndRange 192.168.128.200 -SubnetMask 255.255.255.0
    # Set-DhcpServerV4OptionValue -DnsServer 192.168.128.15 -Router 192.168.128.1
    # Set-DhcpServerv4Scope -ScopeId 192.168.128.15 -LeaseDuration 1.00:00:00
    # Add-DhcpServerInDC -DnsName "meraki-dc1.meraki.airideas.net" -IPAddress 192.168.128.15
...
$vSwitch = "team-1"
$VHDXPath = "C:\vm\gold\server-2016" # location of gold image
...
$Win10VHDXPath = "C:\vm\gold\windows-10"
...
# DC Server Settings Below - changed to elg for lab setup
...
$NewAdminUserName = "anders"
$NewAdminUserPWord = ConvertTo-SecureString -String "P@ssw0rd" -AsPlainText -Force
$ServerVlan = 128
$WorkstationVlan = 128

# File Server Settings Below - changed to meraki for lab setup
...
# Windows 10 Settings - changed to elg for lab setup
# $W10Name = "$CompanyName-win10-1"
$W10Name = "meraki-win10-1"
...
Set-VMNetworkAdapterVlan -VMName $DCVMName -Access -VlanId $ServerVlan 
...
Invoke-Command -VMName $DCVMName -Credential $DCLocalCredential -ScriptBlock {
    ...
    New-NetRoute -InterfaceAlias "Ethernet 2" -DestinationPrefix "0.0.0.0/0" -NextHop "192.168.128.1" | Out-Null
    ...
    Write-Host $DCEffectiveIP | Format-List -Verbose 
...
Write-Verbose "PowerShell Direct responding on VM [$DCVMName]. Setting DNS Fowarders" -Verbose
Invoke-Command -VMName $DCVMName -Credential $DomainUser -ScriptBlock {
    Set-DnsServerForwarder -IPAddress "1.1.1.1","8.8.8.8" -PassThru
    New-GPO -Name "EnableRdp"
    Set-GPLink -Name "EnableRdp" -Target "DC=meraki,DC=airideas,DC=net" -LinkEnabled Yes
    # enable rdp though firewall   
    Set-GPRegistryValue -Name "EnableRdp" -key "SOFTWARE\Policies\Microsoft\Windows NT\Terminal Services" -ValueName fDenyTSConnections -Type REG_DWORD -value "00000000"
    # enable rdp
    Set-GPRegistryValue -Name "EnableRdp" -key "SOFTWARE\Policies\Microsoft\WindowsFirewall\DomainProfile\Services\RemoteDesktop" -ValueName Enabled -Type REG_DWORD -value "00000001"
    Set-GPRegistryValue -Name "EnableRdp" -key "SOFTWARE\Policies\Microsoft\Windows NT\Terminal Services" -ValueName RemoteAddresses -Type REG_SZ -value "*"
...
Set-VMNetworkAdapterVlan -VMName $FSVMName -Access -VlanId $ServerVlan
...
Invoke-Command -VMName $FSVMName -Credential $FSLocalCredential -ScriptBlock {
    ...
    Write-Host $FSEffectiveIP | Format-List -Verbose 
...
Set-VMNetworkAdapterVlan -VMName $W10Name -Access -VlanId $WorkstationVlan
...
Invoke-Command -VMName $W10Name -Credential $FSLocalCredential -ScriptBlock {
    ...
    get-netadapter | Where-Object {$_.name -like "Ethernet*"} | Rename-NetAdapter -NewName "Ethernet-1"
    New-NetIPAddress -IPAddress "$W10IP" -InterfaceAlias "Ethernet-1" -PrefixLength "$SubMaskBit" | Out-Null
    New-NetRoute -InterfaceAlias "Ethernet-1" -DestinationPrefix "0.0.0.0/0" -NextHop "10.10.2.1"  | Out-Null
    $W10EffectiveIP = Get-NetIPAddress -InterfaceAlias "Ethernet-1" | Select-Object IPAddress
    Write-Verbose "Assigned IPv4 and IPv6 IPs for VM [$W10Name] are as follows" -Verbose 
    Write-Host $W10EffectiveIP " is workstation ip." | Format-List -Verbose
    Write-Verbose "Setting DNS Source to [$DCVMName] with IP [$DCIP]" -Verbose
    Set-DnsClientServerAddress -InterfaceAlias "Ethernet-1" -ServerAddresses "$DCIP"
...
# cleanup
# Stop-VM -TurnOff $W10Name, $DCVMName, $FSVMName
# Remove-VM -Name $W10Name, $DCVMName, $FSVMName -Force  
# Remove-Item -Path H:\vm\$W10Name -Recurse -Force
# Remove-Item -Path H:\vm\$DCVMName -Recurse -Force
# Remove-Item -Path H:\vm\$FSVMName -Recurse -Force
# foreach ($item in $W10Name, $DCVMName, $FSVMName){
#      Remove-Item -Path H:\vm\$item -Recurse -Force
# }
